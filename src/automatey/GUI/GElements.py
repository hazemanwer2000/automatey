    ...
    class Basics:
        ...
        class Tree(Widget, INTERNAL.EventManager):
            ...
            def __init__(self, rootNode:"Widgets.Basics.Tree.Node", header:typing.List[str]):
                ...
                self.qWidget.doubleClicked.connect(self.INTERNAL_onDoubleClicked)
            ...
            def INTERNAL_onDoubleClicked(self, qModelIndex:QtCore.QModelIndex):
                if GUtils.EventHandlers.DoubleClickEventHandler in self.eventHandlers:
                    qItem = self.qWidget.itemFromIndex(qModelIndex)
                    self.eventHandlers[GUtils.EventHandlers.DoubleClickEventHandler].fcn(qItem.INJECTED_node)
        ...
        class HexViewer(Widget, INTERNAL.EventManager):
            ...
            def __init__(self, dataBytes:bytes, startAddress:int, bytesPerLine:int=8, bytesPerAddress:int=4):
                self.qWidget = Widgets.Basics.HexViewer.INTERNAL_QHexViewer(dataBytes, startAddress, bytesPerLine, bytesPerAddress)
                super().__init__(self.qWidget)
                INTERNAL.EventManager.__init__(self)
